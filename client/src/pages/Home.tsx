...
const journeyChapters = [
  ...
  { key: "community", phase: "Release", title: "Create a new lineage", body: "Projects can become discoverable, forkable, collaborative product lineages—without destructive copying or broken history.", environment: "community ecosystem" },
...
function WireframeMap({ onBack, onJourney, onJoin }: { onBack: () => void; onJourney: () => void; onJoin: () => void }) {
  ...
  return (
    <main className="wireframe-page">
      ...
      <section className="wireframe-intro"><p className="eyebrow"><span className="eyebrow-line" /> Make the journey visible</p><h1>One system.<br /><em>Two ways in.</em></h1><p>Journey condenses Artifex into eight investor states. Explore preserves all 26 detailed product states, plus the four-part landing opening and waitlist conversion.</p><div className="wireframe-actions"><button className="button button-dark" onClick={onJourney}>Play the cinematic journey <ArrowUpRight size={16} /></button><button className="button button-outline" onClick={onJoin}>Join Artifex Waitlist <ArrowUpRight size={16} /></button></div><div className="experience-balance"><div className="balance-copy"><span>EXPERIENCE MODEL</span><b>8-state investor Journey / 26-state Explore</b><small>Journey is the controlled investment story. Explore keeps every product capability available for deeper inspection.</small></div><div className="mode-switch"><button className={mode === "guided" ? "active" : ""} onClick={() => setMode("guided")}>Journey / 08</button><button className={mode === "explore" ? "active" : ""} onClick={() => setMode("explore")}>Explore / 26</button></div></div><div className="garment-continuity-note"><span className="continuity-dot" /><span><b>Same garment throughout.</b> Every screen changes the state of one evolving digital garment.</span></div><div className="garment-continuity-note copilot-continuity-note"><span className="copilot-mini-orb"><Sparkles size={11} /></span><span><b>One conversation throughout.</b> Ask Artifex remains available across creative, engineering, materials, production, cost, sourcing, documentation, and collection work.</span></div></section>
...
function Journey({ onBack, onJoin }: { onBack: () => void; onJoin: () => void }) {
  ...
  const activeChapters = mode === "guided" ? investorJourneyChapters : journeyChapters;
  ...
  }, [mode, activeChapters.length]);
  const selectMode = (nextMode: "guided" | "explore") => {
    if (nextMode === mode) return;
    setMode(nextMode); setActive(0); setSelectedRegion(""); setCopilotAction("");
  ...
  return (
    <main className={`journey-page immersive-journey mode-${mode} env-${key}`}>
      <div className="immersive-sticky-shell">
      <div className="journey-topbar immersive-topbar"><button className="back-link" onClick={onBack}><ArrowLeft size={16} /> Leave Artifex</button><span className="journey-label">ARTIFEX &nbsp; {mode === "guided" ? "INVESTOR JOURNEY / LOOK 07" : "EXPLORE / LOOK 07"}</span><div className="autonomous-status"><span /> Autonomy / {autonomy}</div><button className="journey-join" onClick={onJoin}>Join waitlist <ArrowUpRight size={15} /></button></div>
      <div className="product-tabs"><span className={chapter.phase === "Creative" ? "active" : ""}>Creative</span><span className={chapter.phase === "Engineering" ? "active" : ""}>Engineering</span><span className={chapter.phase === "Materials" ? "active" : ""}>Materials</span><span className={chapter.phase === "Production" ? "active" : ""}>Production</span><span className={chapter.phase === "Release" || chapter.phase === "Collection" ? "active" : ""}>Release</span><div className="journey-mode"><button className={mode === "guided" ? "active" : ""} onClick={() => selectMode("guided")}>● Journey <small>08</small></button><button className={mode === "explore" ? "active" : ""} onClick={() => selectMode("explore")}>○ Explore <small>26</small></button></div></div>
      <div className="immersive-progress"><span>{String(active + 1).padStart(2, "0")} / {activeChapters.length}</span><TimelineProgress active={active} total={activeChapters.length} /><span>{chapter.environment} · {mode === "guided" ? "controlled investor story" : "full product exploration"}</span></div>
      {active === 0 && <div className="journey-wedge-note"><span>{mode === "guided" ? "EIGHT-STATE INVESTOR JOURNEY" : "CANDIDATE FIRST WEDGE"}</span><b>{mode === "guided" ? "The clearest Artifex story—without the feature maze" : "Constrained dress development"}</b><small>{mode === "guided" ? "Explore remains available for every detailed capability and state." : "Structured intent → fit context → pattern options → material behaviour → living documentation"}</small></div>}
      <section className="immersive-layout">
        ...
        <div className={`persistent-stage ${copilotPulse ? "copilot-changing" : ""}`} onPointerDown={(e) => { dragging.current = true; (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId); }} onPointerMove={(e) => { if (dragging.current) setRotation((r) => r + e.movementX * .45); }} onPointerUp={() => { dragging.current = false; }}>
          ...
          {mode === "guided" && <InvestorStage chapterKey={key as InvestorJourneyKey} graphMode={graphMode} onGraphMode={setGraphMode} material={material} onMaterial={setMaterial} pattern={pattern} onPattern={setPattern} optimise={optimise} onOptimise={setOptimise} engineeringView={engineeringView} onEngineeringView={setEngineeringView} productionOptimised={productionOptimised} onOptimiseProduction={() => { setProductionOptimised(true); setDocumentsReady(true); setCostPath("03"); }} missionFiltered={missionFiltered} onMissionFilter={() => setMissionFiltered(true)} xray={xray} />}
          ...
          {key === "community" && <div className="lineage-network"><div className="lineage-origin">GARMENT<br />001</div><span>ORIGINAL<br /><small>approved</small></span><span>DRAPE 02<br /><small>exploration</small></span><span>FIT TEST<br /><small>branch</small></span><span>COLLAB<br /><small>shared</small></span><div className="lineage-lines" /></div>}
        ...
        <aside className="action-panel">
          ...
          {key === "community" && <><h3>Community lineage</h3><p>Discover, fork, remix, collaborate, or hire without destructive copying or lost attribution.</p><button className="action-primary" onClick={() => go(25)}>Resolve garment</button></>}
      ...
      <section className="immersive-timeline"><div className="timeline-scroller">{activeChapters.map((item, index) => <button key={item.key} className={index === active ? "active" : ""} onClick={() => go(index)}><span>{String(index + 1).padStart(2,"0")}</span><b>{item.key.replace("investor-", "")}</b></button>)}</div><div className="journey-footnote"><Sparkles size={16} /><span>{mode === "guided" ? "Eight controlled states. Switch to Explore for the full 26-state product simulation." : "Full 26-state product simulation. Switch to Journey for the investor story."}</span></div></section>
      </div>
    </main>
  );
}
