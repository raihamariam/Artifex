...
const journeyChapters = [
  ...
  { key: "versions", phase: "Collaboration", title: "Design together, move faster", body: "Version, comment, and approve around the same garment without losing the creative or production context.", environment: "collaborative development" },
  ...
  { key: "engineering", phase: "Engineering", title: "From form to function", body: "Understand how the garment is constructed with pattern engineering, construction logic, and controlled alternatives.", environment: "form to function" },
  ...
  { key: "materials", phase: "Materials", title: "Materials change everything", body: "Explore how different fabrics alter form, drape, fit, and construction while staying attached to Garment / 001.", environment: "material intelligence" },
  ...
  { key: "production", phase: "Production", title: "From design to production", body: "Optimise for real-world manufacturing with marker efficiency, construction sequence, BOM, and measurable material decisions.", environment: "design to manufacturing" },
  ...
  { key: "release", phase: "Release", title: "A complete garment. A connected product.", body: "Every design, data, and document is unified and ready for the next stage of production.", environment: "complete product" },
  ...
  { key: "community", phase: "Community", title: "Build what’s next together.", body: "Discover, fork, remix, and collaborate on structured garments while creator credit and editable engineering knowledge remain intact.", environment: "community ecosystem" },
] as const;
...
function Garment({ mode = "hero", material = "silver", pattern = false }: { mode?: "hero" | "studio" | "mini"; material?: string; pattern?: boolean }) {
  return (
    <div className={`garment-scene ${mode} material-${material} ${pattern ? "is-pattern" : ""}`}>
      {mode !== "hero" && <img className="journey-canonical-garment" src="/manus-storage/artifex-reference-model-clean_2d14de69.png" alt="Artifex silver sculptural garment" />}
...
function ProductGraphScene({ onEnter }: { onEnter: () => void }) {
  ...
  return (
    <main className="landing-page">
      ...
      <section className="principle-section" id="principle">
        ...
        <div className="principle-visual graph-expanded supplied-figure-graph"><ProductGraphScene onEnter={onJourney} /></div>
...
function PersistentCopilot({ chapterKey, role, selectedContext, onRole, onExecute }: { chapterKey: JourneyKey; role: string; selectedContext?: string; onRole: (role: string) => void; onExecute: (prompt: string) => void }) {
  ...
  return <section className="persistent-copilot" onPointerDown={(event) => event.stopPropagation()}>
    ...
    <div className="copilot-tools"><button onClick={() => setInput("Interpret this sketch while preserving the silver Liquid Architecture silhouette.")}><ImagePlus size={13} /> Sketch</button><button onClick={() => setInput("Read this reference photo and map its intent onto Garment / 001.")}><ImagePlus size={13} /> Photo</button><button onClick={() => setInput("Test a material change and show its impact on drape, fit, pattern, and cost.")}><Layers3 size={13} /> Material</button><button onClick={() => setInput("Refresh the tech pack and documentation from the current garment state.")}><ImagePlus size={13} /> Document</button><button onClick={() => setInput("Use a voice-style instruction: keep the collar architectural and the waist drape fluid.")}><Mic size={13} /> Voice</button></div>
...
function ReferenceBoard({ chapterKey, branch, onBranch, patternPiece, onPatternPiece, material, onMaterial, productionOptimised, onProductionOptimise, forked, onFork, hoveredRegion, onRegion }: { chapterKey: JourneyKey; branch: string; onBranch: (branch: string) => void; patternPiece: string; onPatternPiece: (piece: string) => void; material: string; onMaterial: (material: string) => void; productionOptimised: boolean; onProductionOptimise: () => void; forked: boolean; onFork: () => void; hoveredRegion: string; onRegion: (region: string) => void }) {
  const is = (...keys: JourneyKey[]) => keys.includes(chapterKey);
  if (is("creative", "investor-idea", "investor-creative", "understanding", "fit")) return <div className="reference-board reference-creative"><div className="ref-board-title">MOODBOARD / CREATIVE EDITORIAL</div><div className="ref-thumb thumb-one" /><div className="ref-thumb thumb-two" /><div className="ref-thumb thumb-three" /><div className="ref-sketch"><span>EARLY FASHION SKETCH / EXACT HERO GARMENT</span><i /><i /><i /></div><div className="ref-interpretation"><span>{chapterKey === "fit" ? "3D CREATIVE CONCEPT" : "INTERPRET CREATIVE INTENT"}</span><Garment mode="mini" material="silver" /></div><img className="ref-hero-model creative-hero-model" src="/manus-storage/artifex-reference-model-clean_2d14de69.png" alt="Artifex hero garment" /></div>;
  if (chapterKey === "versions") return <div className="reference-board reference-collaboration"><button type="button" className={`ref-branch-card main ${branch === "mainline" ? "selected" : ""}`} onClick={() => onBranch("mainline")}><b>MAINLINE</b><small>v1.4 · approved</small><Garment mode="mini" material="silver" /></button><button type="button" className={`ref-branch-card ${branch === "creative" ? "selected" : ""}`} onClick={() => onBranch("creative")}><b>CREATIVE BRANCH</b><small>v1.5-C · longer drape · collar expression</small><Garment mode="mini" material="silver" /></button><button type="button" className={`ref-branch-card ${branch === "production" ? "selected" : ""}`} onClick={() => onBranch("production")}><b>TECHNICAL / PRODUCTION</b><small>v1.5-T · collar support · sleeve mobility</small><Garment mode="mini" material="silver" /></button><div className="ref-comments"><b>COMMENTS / REVIEW</b><span>Creative Director<small>COLLAR — Preserve this volume.</small></span><span>Pattern Engineer<small>SLEEVE — Improve mobility without losing scale.</small></span><span>Product Manager<small>WAIST — Simplify the seam without changing tension.</small></span><span>Designer<small>DRAPE — Keep this panel fluid.</small></span><button type="button" className="ref-merge" onClick={() => onBranch("mainline")}>MERGE CANDIDATE →</button></div></div>;
  if (is("understanding", "fit", "engineering", "pattern", "impact", "construction", "xray")) { const pieces = ["P01", "P02", "P03", "P04", "P05", "P06", "P07", "P08", "P09", "P10", "P11", "P12", "P13", "P14", "P15"]; return <div className="reference-board reference-engineering"><div className="ref-engineering-model"><img className={`ref-hero-model engineering-hero-model ${hoveredRegion ? "region-highlight" : ""}`} src="/manus-storage/artifex-reference-model-clean_2d14de69.png" alt="Artifex technical garment" /><button type="button" onMouseEnter={() => onRegion("collar")} onMouseLeave={() => onRegion("")}>SCULPTURAL COLLAR</button><button type="button" onMouseEnter={() => onRegion("sleeve")} onMouseLeave={() => onRegion("")}>VOLUMETRIC SLEEVE</button><button type="button" onMouseEnter={() => onRegion("waist")} onMouseLeave={() => onRegion("")}>WAIST / TWIST</button><button type="button" onMouseEnter={() => onRegion("drape")} onMouseLeave={() => onRegion("")}>DRAPE PANEL</button></div><div className="ref-pattern-grid">{pieces.map((piece) => <button type="button" className={patternPiece === piece ? "selected" : ""} onClick={() => onPatternPiece(piece)} key={piece}><i /><small>{piece}</small></button>)}</div><div className="ref-engineering-tabs"><b>3D GARMENT</b><b>SEAM TOPOLOGY</b><b className="active">TECHNICAL FLAT / PATTERN SYSTEM</b><b>CONSTRUCTION ZONES</b></div></div>; }
  if (is("materials", "textile", "propagation", "surface")) return <div className="reference-board reference-materials"><button type="button" className={`ref-material-card ${material === "silver" ? "selected" : ""}`} onClick={() => onMaterial("silver")}><Garment mode="mini" material="silver" /><b>LIQUID METALLIC LAMINATE</b><small>structural retention · sharp fold memory</small></button><button type="button" className={`ref-material-card ${material === "matte" ? "selected" : ""}`} onClick={() => onMaterial("matte")}><Garment mode="mini" material="matte" /><b>SOFT METALLIC SILK BLEND</b><small>deeper drape · softer sleeves</small></button><button type="button" className={`ref-material-card dark ${material === "ink" ? "selected" : ""}`} onClick={() => onMaterial("ink")}><Garment mode="mini" material="ink" /><b>MATTE TECHNICAL TEXTILE</b><small>clear seam visibility · restrained surface</small></button><div className="ref-impact-list"><b>MATERIAL IMPACT</b><span>◉ Drape behaviour</span><span>◉ Collar rigidity</span><span>◉ Pattern ease</span><span>◉ Construction method</span><span>◉ Cost implication</span><span>◉ Production stability</span></div></div>;
  if (is("production", "bom", "sampling", "sourcing", "costing")) { const pieces = ["P01", "P02", "P03", "P04", "P05", "P06", "P07", "P08", "P09", "P10", "P11", "P12", "P13", "P14", "P15"]; return <div className="reference-board reference-production"><button type="button" className={`ref-marker ${productionOptimised ? "selected" : ""}`} onClick={onProductionOptimise}><b>MARKER LAYOUT / CANONICAL PATTERN FAMILY</b><div className="marker-roll initial">{pieces.map((piece) => <i className="marker-piece" key={`initial-${piece}`}>{piece}</i>)}</div><div className="marker-roll optimised">{pieces.slice().reverse().map((piece) => <i className="marker-piece" key={`optimised-${piece}`}>{piece}</i>)}</div><span>{productionOptimised ? "68% utilisation → 87% utilisation" : "68% utilisation → optimise"}</span></button><div className="ref-sequence"><b>CONSTRUCTION SEQUENCE</b>{["Base assembly", "Sleeves", "Collar", "Waist", "Drape", "Finishing"].map((item, i) => <span key={item}><i>{String(i + 1).padStart(2, "0")}</i>{item}</span>)}</div><div className="ref-bom"><b>BOM / EST.</b><span>Main fabric <em>2.8m</em></span><span>Lining <em>1.5m</em></span><span>Interfacing <em>0.8m</em></span><span>Thread <em>1 set</em></span><strong>DOCUMENTATION<br />GENERATED</strong></div></div>; }
  if (is("release", "release-ready", "ending")) return <div className="reference-board reference-release"><div className="ref-connected-product"><img className="ref-hero-model release-hero-model" src="/manus-storage/artifex-reference-model-clean_2d14de69.png" alt="Artifex hero garment" /><b>ARTIFEX HERO 001</b></div><div className="ref-release-links">{["Creative intent", "3D model", "2D pattern", "Fit data", "Material spec", "Construction", "BOM & costing", "Tech pack", "QC & testing", "Version history"].map((item) => <span key={item}>● {item}<em>Ready</em></span>)}</div><div className="ref-release-status"><b>RELEASE PACKAGE</b><span>All connected product intelligence</span><strong>APPROVED</strong></div></div>;
  if (chapterKey === "community") return <div className="reference-board reference-community"><button type="button" className="ref-community-feature" onClick={onFork}><img className="ref-hero-model community-hero-model" src="/manus-storage/artifex-reference-model-clean_2d14de69.png" alt="Artifex hero garment" /><b>{forked ? "NEW GARMENT LINEAGE" : "ARTIFEX HERO 001"}</b><small>{forked ? "forked from Artifex Hero 001 · editable" : "by Artifex · structured project"}</small></button>{["Ready-to-wear fork", "Drape study", "Material study", "Editorial remix"].map((item, i) => <button type="button" className="ref-community-card" onClick={onFork} key={item}><Garment mode="mini" material={i === 2 ? "matte" : "silver"} /><span>{forked && i === 0 ? "Your new fork" : item}</span><small>♡ {1.2 - i * .2}k · Fork</small></button>)}</div>;
  if (chapterKey === "conversation" || chapterKey === "autonomy" || chapterKey === "evidence") return <div className="reference-board reference-copilot"><div className="ref-copilot-garment"><Garment mode="studio" material="silver" /></div><div className="ref-chat"><span>ASK ARTIFEX</span><p>How can I make the collar more architectural without affecting the drape?</p><b>Artifex</b><p className="reply">Increase collar height and refine the internal support structure. The change preserves the waist twist and fluid panel.</p><div className="ref-options"><i /><i /><i /></div></div></div>;
  const utility: Record<string, [string, string, string]> = {
    understanding: ["GARMENT READING", "REGIONS / DRAPE / WAIST", "STRUCTURED PRODUCT MODEL"],
    fit: ["FIT CONTEXT", "BODY / EASE / TENSION", "SIZE SET REVIEW"],
    construction: ["CONSTRUCTION ZONES", "ASSEMBLY / SEAM / CLOSURE", "SEQUENCE PREVIEW"],
    knowledge: ["KNOWLEDGE GRAPH", "HISTORY / EVIDENCE / QC", "COMPOUNDING PRODUCT TRUTH"],
    "release-ready": ["RELEASE GATE", "APPROVAL / PACKAGE / STATUS", "CONNECTED PRODUCT READY"],
    mission: ["MISSION CONTROL", "COLLECTION / RISK / READINESS", "LOOKS THAT NEED ATTENTION"],
    autonomy: ["BOUNDED AUTONOMY", "OBSERVE / SUGGEST / EXECUTE", "POLICY + HUMAN CONTROL"],
    evidence: ["DECISION RECEIPT", "RULE / ACTION / IMPACT", "TRACEABLE AND REVERSIBLE"],
    ending: ["ONE GARMENT", "CREATIVE / ENGINEERING / KNOWLEDGE", "A CONNECTED SYSTEM"]
  };
  const copy = utility[chapterKey] || [chapterKey.toUpperCase(), "CONNECTED PRODUCT STATE", "ARTIFEX / GARMENT 001"];
  return <div className="reference-board reference-utility"><div className="utility-garment"><Garment mode="studio" material="silver" /></div><div className="utility-stack"><span>{copy[0]}</span><b>{copy[1]}</b><small>{copy[2]}</small><div className="utility-metrics"><i /><i /><i /><i /><i /><i /></div></div><div className="utility-cards"><article><b>INPUT</b><span>Creative intent</span><small>captured</small></article><article><b>RELATIONSHIPS</b><span>Fit · material · pattern</span><small>linked</small></article><article><b>OUTPUT</b><span>Product documentation</span><small>updating</small></article></div></div>;
}
...
function Journey({ onBack, onJoin }: { onBack: () => void; onJoin: () => void }) {
  ...
  const [branch, setBranch] = useState("mainline");
  const [patternPiece, setPatternPiece] = useState("P01");
  ...
  const [productionVolume, setProductionVolume] = useState("10K");
  ...
  const signal = (message: string) => {
    ...
    window.setTimeout(() => setActionFeedback((current) => current === message ? "" : current), 2200);
  };
  useEffect(() => {
  ...
  const executeCopilot = (prompt: string) => {
    ...
    if (key === "community") { setForked(true); setSelectedRegion("Drape lineage / GARMENT 001 / non-destructive branch"); }
  ...
  return (
    <main className={`journey-page immersive-journey mode-explore env-${key}`}>
      ...
      <div className="immersive-progress"><span>{String(active + 1).padStart(2, "0")} / {activeChapters.length}</span><TimelineProgress active={active} total={activeChapters.length} /><span>{chapter.environment} · full product exploration</span></div>
        ...
        <div className="journey-rail" aria-hidden="true"><span>✦</span><span>◫</span><span>⌁</span><span>◌</span><span>↗</span></div>
        ...
        <div className={`persistent-stage colour-${colour} stage-${key} ${["creative", "versions", "engineering", "pattern", "impact", "xray", "materials", "textile", "propagation", "surface", "production", "bom", "sampling", "sourcing", "costing", "release", "release-ready", "ending", "community", "conversation", "autonomy", "evidence"].includes(key as string) ? "reference-primary" : ""} ${copilotPulse ? "copilot-changing" : ""}`} style={{ "--garment-rotation": `${rotation}deg` } as React.CSSProperties} onPointerDown={(e) => { dragging.current = true; (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId); }} onPointerMove={(e) => { if (!dragging.current) return; dragDelta.current += e.movementX * .45; if (dragFrame.current === null) { dragFrame.current = window.requestAnimationFrame(() => { dragFrame.current = null; const delta = dragDelta.current; dragDelta.current = 0; setRotation((r) => r + delta); }); } }} onPointerUp={() => { dragging.current = false; }} onPointerCancel={() => { dragging.current = false; }} onLostPointerCapture={() => { dragging.current = false; }}>
          <div className="stage-grid" /><div className="runway-floor" /><div className="garment-rotator" style={{ transform: `translate(-50%, -50%) rotateY(${rotation}deg) scale(${key === "fit" ? .82 + (size - 6) * .012 : 1})` }}><Garment mode="studio" material={materialForStage} pattern={key === "pattern" || key === "production" || key === "investor-production"} /></div><div className="stage-identity"><span>GARMENT / 001</span><span>drag to rotate ↔</span></div><ReferenceBoard chapterKey={key} branch={branch} onBranch={setBranch} patternPiece={patternPiece} onPatternPiece={setPatternPiece} material={material} onMaterial={setMaterial} productionOptimised={productionOptimised} onProductionOptimise={() => setProductionOptimised(true)} forked={forked} onFork={() => setForked(true)} hoveredRegion={hoveredRegion} onRegion={setHoveredRegion} />
          ...
          {key === "sourcing" && <div className="supplier-cards"><button type="button" className={supplier === "A" ? "active" : ""} onClick={() => { setSupplier("A"); signal("Supplier A selected for Garment / 001."); }}>A<small>$18.40/m · 18 days · low risk</small></button><button type="button" className={supplier === "B" ? "active" : ""} onClick={() => { setSupplier("B"); signal("Supplier B selected for Garment / 001."); }}>B<small>$16.70/m · 37 days · medium risk</small></button><button type="button" className={supplier === "C" ? "active" : ""} onClick={() => { setSupplier("C"); signal("Supplier C selected for Garment / 001."); }}>C<small>$20.10/m · 11 days · low risk</small></button></div>}
          ...
          {key === "community" && <div className="community-scene"><div className="community-label"><span>COMMUNITY / ECOSYSTEM</span><b>Share structured fashion work.</b><small>Discover projects, fork garments, collaborate with specialists, and grow from one evolving product model.</small></div><div className="community-ecosystem"><div className="community-project-card project-vest"><div className="community-project-garment"><Garment mode="mini" material="silver" /></div>DRAPED VEST<small>by Mara Bell</small></div><div className="community-project-card project-top"><div className="community-project-garment"><Garment mode="mini" material="silver" /></div>SCULPTED TOP<small>by Priya N.</small></div><div className="community-project-card project-pant"><div className="community-project-garment"><Garment mode="mini" material="silver" /></div>UTILITY PANT<small>by Studio KRN</small></div><div className="community-project-card project-coat"><div className="community-project-garment"><Garment mode="mini" material="silver" /></div>ASYMMETRIC COAT<small>by Rene CAD</small></div><div className="community-creator creator-discover">◉<b>DISCOVER</b><small>New projects<br />& creators</small></div><div className="featured-project"><span>FEATURED PROJECT</span><Garment mode="mini" material="silver" /><b>MODULAR SHELL JACKET</b><small>by Alex Chen · technical modular · recyclable</small></div><div className="community-creator creator-share">◇<b>SHARE</b><small>Publish your<br />structured work</small></div><div className="community-creator creator-fork">⌘<b>FORK</b><small>Build new<br />variations</small></div><div className="community-creator creator-remix">△<b>REMIX</b><small>Explore<br />new ideas</small></div><div className="community-creator creator-collaborate">◉<b>COLLABORATE</b><small>Work with<br />specialists</small></div><div className="community-creator creator-hire">♧<b>HIRE</b><small>Find makers<br />& factories</small></div><div className="creator-lineage">CREATOR ATTRIBUTION <b>Alex Chen · original project</b><small>Forks preserve credit, history, and editable garment intelligence.</small></div></div></div>}
        ...
        <aside className="action-panel">
          ...
          {key === "production" && <><h3>Marker efficiency</h3><p>{productionOptimised ? "Fabric −0.31m / Waste −15.6% / Cost −$4.82" : "Current utilisation 72.8%"}</p><button className="action-primary" onClick={() => { setProductionOptimised(true); signal("Marker optimised: 88.4% utilisation."); }}>{productionOptimised ? "Optimised / 88.4%" : "Optimise"}</button></>}
          {key === "bom" && <><h3>Live BOM</h3><p>Shell 2.41m<br />Lining 1.33m<br />Interfacing 0.44m<br />Invisible zipper 1</p><button className="action-primary" onClick={() => { setDocumentsReady(true); signal("Factory review package refreshed from Garment / 001."); }}>Generate documentation</button></>}
          ...
          {key === "sourcing" && <><h3>Supplier {supplier} selected</h3><p>{supplier === "C" ? "Lower risk / +4 days / MOQ 250m" : "Cost ↓ / Lead time ↑ / MOQ ↑ / Risk ↑"}</p><button className="action-primary" onClick={() => { setCostPath("03"); signal(`Supplier ${supplier} locked to the garment product graph.`); }}>Maintain change <Check size={14} /></button></>}
          {key === "costing" && <><h3>{costPath ? "Target achieved / $113.90" : "Production volume"}</h3><div className="choice-row">{["1K", "5K", "10K", "50K"].map((item) => <button className={productionVolume === item ? "selected" : ""} onClick={() => { setProductionVolume(item); signal(`Cost view updated for ${item} units.`); }} key={item}>{item}</button>)}</div><p>{costPath ? `Path 03 combines trim and marker optimisation at ${productionVolume} units while keeping the shell fabric.` : `Pattern optimisation saved $4.82 per garment at ${productionVolume} units.`}</p></>}
          {key === "release" && <><h3>Approval + release</h3><select value={role} onChange={(e) => setRole(e.target.value)}><option>Technical Designer</option><option>Production Approver</option></select><button className="action-primary" disabled={role !== "Production Approver"} onClick={() => { setReleased(true); signal("Approved product state released to production."); }}>Release to production</button></>}
          {key === "mission" && <><h3>AW27 collection</h3><p>42 products<br />31 production ready<br />6 engineering<br />3 review<br />2 at risk</p><button className="action-primary" onClick={() => { setMissionFiltered(false); go(0); signal("Returned to Garment / 001."); }}>Return to garment 08</button></>}
          ...
          {key === "community" && <><span className="panel-label">PROJECT / COMMUNITY</span><h3>Community scene</h3><p>A shared future for structured fashion.</p><div className="community-project-preview"><div className="preview-garment"><Garment mode="mini" material="silver" /></div><div><b>Garment / 001</b><small>by Artifex <i>●</i></small></div></div><div className="community-meta"><span>Garment type <b>Outerwear</b></span><span>Materials <b>Silver sculptural shell</b></span><span>Status <b><i>●</i> Structured project</b></span><span>Availability <b>Fork, Collaborate, Hire</b></span></div><div className="community-actions"><button onClick={() => signal("Fork created: Garment / 001 → your new lineage.")}>Fork garment <ArrowRight size={13} /></button><button onClick={() => signal("Collaboration request attached to the garment.")}>Request collaboration <ArrowRight size={13} /></button><button onClick={() => signal("Maker brief opened for Garment / 001.")}>Hire maker / factory <ArrowRight size={13} /></button><button onClick={() => signal("Garment / 001 saved to your collection.")}>Save to collection <ArrowRight size={13} /></button></div><div className="community-metrics"><b>12<small>forks</small></b><b>4<small>collaborators</small></b><b>3<small>material matches</small></b><b>2<small>factories interested</small></b></div><button className="action-primary" onClick={() => go(25)}>Continue to ending <ArrowRight size={14} /></button></>}
          ...
          {actionFeedback && <div className="action-feedback" role="status">{actionFeedback}</div>}<button className="next-control" onClick={() => active === activeChapters.length - 1 ? onJoin() : go(active + 1)}>{active === activeChapters.length - 1 ? "Finish journey" : "Continue"}<ArrowUpRight size={16} /></button>
        </aside>
      </section>
