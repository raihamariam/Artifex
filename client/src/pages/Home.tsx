...
function ReferenceBoard({ chapterKey, branch, onBranch, patternPiece, onPatternPiece, material, onMaterial, productionOptimised, onProductionOptimise, forked, onFork, hoveredRegion, onRegion }: { chapterKey: JourneyKey; branch: string; onBranch: (branch: string) => void; patternPiece: string; onPatternPiece: (piece: string) => void; material: string; onMaterial: (material: string) => void; productionOptimised: boolean; onProductionOptimise: () => void; forked: boolean; onFork: () => void; hoveredRegion: string; onRegion: (region: string) => void }) {
  ...
  if (is("creative", "investor-idea", "investor-creative")) return <div className="reference-board reference-creative"><div className="ref-board-title">MOODBOARD</div><div className="ref-thumb thumb-one" /><div className="ref-thumb thumb-two" /><div className="ref-thumb thumb-three" /><div className="ref-sketch"><span>INTENT SKETCH</span><i /><i /><i /></div><div className="ref-interpretation"><span>INTERPRETATION</span><Garment mode="mini" material="silver" /></div><img className="ref-hero-model creative-hero-model" src="/manus-storage/artifex-reference-model-clean_2d14de69.png" alt="Artifex hero garment" /></div>;
  if (chapterKey === "versions") return <div className="reference-board reference-collaboration"><button type="button" className={`ref-branch-card main ${branch === "mainline" ? "selected" : ""}`} onClick={() => onBranch("mainline")}><b>MAINLINE</b><small>v1.4 · approved</small><Garment mode="mini" material="silver" /></button><button type="button" className={`ref-branch-card ${branch === "creative" ? "selected" : ""}`} onClick={() => onBranch("creative")}><b>CREATIVE BRANCH</b><small>v1.5-C · longer drape</small><Garment mode="mini" material="silver" /></button><button type="button" className={`ref-branch-card ${branch === "production" ? "selected" : ""}`} onClick={() => onBranch("production")}><b>PRODUCTION BRANCH</b><small>v1.5-T · simplified construction</small><Garment mode="mini" material="silver" /></button><div className="ref-comments"><b>COMMENTS</b><span>Creative Director<small>Preserve the collar volume.</small></span><span>Pattern Engineer<small>Can we simplify the seam?</small></span><span>Product Manager<small>Check sleeve mobility.</small></span><span>Designer<small>This version looks strong. Let’s merge.</small></span><button type="button" className="ref-merge" onClick={() => onBranch("mainline")}>MERGE TO MAINLINE →</button></div></div>;
  if (is("engineering", "pattern", "impact", "xray")) { const pieces = ["P01", "P02", "P03", "P04", "P05", "P06", "P07", "P08", "P09", "P10", "P11", "P12", "P13", "P14", "P15"]; return <div className="reference-board reference-engineering"><div className="ref-engineering-model"><img className={`ref-hero-model engineering-hero-model ${hoveredRegion ? "region-highlight" : ""}`} src="/manus-storage/artifex-reference-model-clean_2d14de69.png" alt="Artifex technical garment" /><button type="button" onMouseEnter={() => onRegion("collar")} onMouseLeave={() => onRegion("")}>SCULPTURAL COLLAR</button><button type="button" onMouseEnter={() => onRegion("sleeve")} onMouseLeave={() => onRegion("")}>VOLUMETRIC SLEEVE</button><button type="button" onMouseEnter={() => onRegion("waist")} onMouseLeave={() => onRegion("")}>WAIST TWIST</button><button type="button" onMouseEnter={() => onRegion("drape")} onMouseLeave={() => onRegion("")}>ASYMMETRIC DRAPE</button></div><div className="ref-pattern-grid">{pieces.map((piece) => <button type="button" className={patternPiece === piece ? "selected" : ""} onClick={() => onPatternPiece(piece)} key={piece}><i /><small>{piece}</small></button>)}</div><div className="ref-engineering-tabs"><b>3D GARMENT</b><b>SEAM TOPOLOGY</b><b className="active">20 PATTERN PIECES</b><b>CONSTRUCTION</b></div></div>; }
  if (is("materials", "textile", "propagation", "surface")) return <div className="reference-board reference-materials"><button type="button" className={`ref-material-card ${material === "silver" ? "selected" : ""}`} onClick={() => onMaterial("silver")}><Garment mode="mini" material="silver" /><b>LIQUID METALLIC</b><small>high structure · sharp folds</small></button><button type="button" className={`ref-material-card ${material === "matte" ? "selected" : ""}`} onClick={() => onMaterial("matte")}><Garment mode="mini" material="matte" /><b>SOFT METALLIC BLEND</b><small>fluid drape · softer silhouette</small></button><button type="button" className={`ref-material-card dark ${material === "ink" ? "selected" : ""}`} onClick={() => onMaterial("ink")}><Garment mode="mini" material="ink" /><b>MATTE TECHNICAL</b><small>engineering view · clear seam definition</small></button><div className="ref-impact-list"><b>MATERIAL IMPACT</b><span>◉ Drape behaviour</span><span>◉ Collar rigidity</span><span>◉ Pattern ease</span><span>◉ Construction method</span><span>◉ Cost implication</span></div></div>;
  if (is("production", "bom", "sampling", "sourcing", "costing")) return <div className="reference-board reference-production"><button type="button" className={`ref-marker ${productionOptimised ? "selected" : ""}`} onClick={onProductionOptimise}><b>MARKER LAYOUT</b><div className="marker-roll initial" /><div className="marker-roll optimised" /><span>{productionOptimised ? "68% utilisation → 87% utilisation" : "68% utilisation → optimise"}</span></button><div className="ref-sequence"><b>CONSTRUCTION SEQUENCE</b>{["Base assembly", "Sleeves", "Collar", "Waist", "Drape", "Finishing"].map((item, i) => <span key={item}><i>{String(i + 1).padStart(2, "0")}</i>{item}</span>)}</div><div className="ref-bom"><b>BOM / EST.</b><span>Main fabric <em>2.8m</em></span><span>Lining <em>1.5m</em></span><span>Interfacing <em>0.8m</em></span><span>Thread <em>1 set</em></span><strong>DOCUMENTATION<br />GENERATED</strong></div></div>;
