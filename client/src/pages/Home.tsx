...
function ReferenceBoard({ chapterKey }: { chapterKey: JourneyKey }) {
  ...
  if (chapterKey === "versions") return <div className="reference-board reference-collaboration"><div className="ref-branch-card main"><b>MAINLINE</b><small>v1.4 · approved</small><Garment mode="mini" material="silver" /></div><div className="ref-branch-card"><b>CREATIVE BRANCH</b><small>v1.5-C · longer drape</small><Garment mode="mini" material="silver" /></div><div className="ref-branch-card"><b>PRODUCTION BRANCH</b><small>v1.5-T · simplified construction</small><Garment mode="mini" material="silver" /></div><div className="ref-comments"><b>COMMENTS</b><span>Creative Director<small>Preserve the collar volume.</small></span><span>Pattern Engineer<small>Can we simplify the seam?</small></span><span>Product Manager<small>Check sleeve mobility.</small></span><span>Designer<small>This version looks strong. Let’s merge.</small></span></div></div>;
  if (is("engineering", "pattern", "impact", "xray")) return <div className="reference-board reference-engineering"><div className="ref-engineering-model"><img className="ref-hero-model engineering-hero-model" src="/manus-storage/artifex-reference-model-clean_2d14de69.png" alt="Artifex technical garment" /><span>SCULPTURAL COLLAR</span><span>VOLUMETRIC SLEEVE</span><span>WAIST TWIST</span><span>ASYMMETRIC DRAPE</span></div><div className="ref-pattern-grid">{Array.from({ length: 12 }).map((_, i) => <i key={i} />)}</div><div className="ref-engineering-tabs"><b>3D GARMENT</b><b>SEAM TOPOLOGY</b><b className="active">20 PATTERN PIECES</b><b>CONSTRUCTION</b></div></div>;
  if (is("materials", "textile", "propagation", "surface")) return <div className="reference-board reference-materials"><div className="ref-material-card"><Garment mode="mini" material="silver" /><b>LIQUID METALLIC</b><small>high structure · sharp folds</small></div><div className="ref-material-card"><Garment mode="mini" material="matte" /><b>SOFT METALLIC BLEND</b><small>fluid drape · softer silhouette</small></div><div className="ref-material-card dark"><Garment mode="mini" material="ink" /><b>MATTE TECHNICAL</b><small>engineering view · clear seam definition</small></div><div className="ref-impact-list"><b>MATERIAL IMPACT</b><span>◉ Drape behaviour</span><span>◉ Collar rigidity</span><span>◉ Pattern ease</span><span>◉ Construction method</span><span>◉ Cost implication</span></div></div>;
  if (is("production", "bom", "sampling", "sourcing", "costing")) return <div className="reference-board reference-production"><div className="ref-marker"><b>MARKER LAYOUT</b><div className="marker-roll initial" /><div className="marker-roll optimised" /><span>68% utilisation → <em>87% utilisation</em></span></div><div className="ref-sequence"><b>CONSTRUCTION SEQUENCE</b>{["Base assembly", "Sleeves", "Collar", "Waist", "Drape", "Finishing"].map((item, i) => <span key={item}><i>{String(i + 1).padStart(2, "0")}</i>{item}</span>)}</div><div className="ref-bom"><b>BOM / EST.</b><span>Main fabric <em>2.8m</em></span><span>Lining <em>1.5m</em></span><span>Interfacing <em>0.8m</em></span><span>Thread <em>1 set</em></span><strong>DOCUMENTATION<br />GENERATED</strong></div></div>;
  ...
  if (chapterKey === "community") return <div className="reference-board reference-community"><div className="ref-community-feature"><img className="ref-hero-model community-hero-model" src="/manus-storage/artifex-reference-model-clean_2d14de69.png" alt="Artifex hero garment" /><b>ARTIFEX HERO 001</b><small>by Artifex · structured project</small></div>{["Ready-to-wear fork", "Drape study", "Material study", "Editorial remix"].map((item, i) => <div className="ref-community-card" key={item}><Garment mode="mini" material={i === 2 ? "matte" : "silver"} /><span>{item}</span><small>♡ {1.2 - i * .2}k · Fork</small></div>)}</div>;
