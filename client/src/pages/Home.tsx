...
const journeyChapters = [
  ...
  { key: "versions", phase: "Creative", title: "Collaborate without losing truth", body: "Designers, technical designers, and pattern makers can branch, compare, and merge decisions while the garment lineage remains intact.", environment: "collaboration workspace" },
  ...
  { key: "community", phase: "Release", title: "Create a new lineage", body: "After release, teams can discover, fork, remix, collaborate, or hire around attributable garment lineages without destructive copying.", environment: "community ecosystem" },
...
function Journey({ onBack, onJoin }: { onBack: () => void; onJoin: () => void }) {
  ...
  return (
    <main className={`journey-page immersive-journey mode-explore env-${key}`}>
      ...
      <div className="product-tabs"><span className={chapter.phase === "Creative" && key !== "versions" ? "active" : ""}>Creative</span><span className={key === "versions" ? "active" : ""}>Collaboration</span><span className={chapter.phase === "Engineering" ? "active" : ""}>Engineering</span><span className={chapter.phase === "Materials" ? "active" : ""}>Materials</span><span className={chapter.phase === "Production" ? "active" : ""}>Production</span><span className={chapter.phase === "Release" && key !== "community" ? "active" : ""}>Release</span><span className={key === "community" ? "active" : ""}>Community</span></div>
      ...
      <section className="immersive-layout">
        ...
        <div className={`persistent-stage ${copilotPulse ? "copilot-changing" : ""}`} onPointerDown={(e) => { dragging.current = true; (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId); }} onPointerMove={(e) => { if (dragging.current) setRotation((r) => r + e.movementX * .45); }} onPointerUp={() => { dragging.current = false; }}>
          ...
          {key === "versions" && <div className="collaboration-scene"><div className="collab-header"><span>COLLABORATION / REPOSITORY</span><b>One garment. Many decisions.</b></div><div className="collab-repository"><div className="collab-branch mainline"><b>MAINLINE</b><small>v1.4.0 · current · 2 open comments</small></div><div className="collab-branch creative-branch"><b>CREATIVE BRANCH</b><small>v1.5.0 · 3 changes</small></div><div className="collab-branch production-branch"><b>PRODUCTION BRANCH</b><small>v1.3.1 · requested change</small></div><div className="collab-garment"><Garment mode="mini" material="silver" /></div><div className="collab-region-note shoulder-note"><i />Preserve shoulder volume.</div><div className="collab-region-note seam-note"><i />Check seam ease.</div><div className="collab-review">REVIEW / APPROVAL <b>Merge candidate A</b><small>All checks passed · material locked</small></div><div className="collab-merge-arrow">↓</div></div><div className="collab-versions"><span>CREATE</span><span className="active">COMPARE VERSIONS</span><span>COMMENT</span><span>REVIEW</span><span>APPROVE</span><span>MERGE</span><span>ROLLBACK</span></div></div>}
          ...
          {key === "community" && <div className="community-scene"><div className="community-label"><span>COMMUNITY / ECOSYSTEM</span><b>Share structured fashion work.</b><small>Discover projects, fork garments, collaborate with specialists, and grow from one evolving product model.</small></div><div className="community-ecosystem"><div className="featured-project"><span>FEATURED PROJECT</span><Garment mode="mini" material="matte" /><b>MODULAR SHELL JACKET</b><small>by Artifex creator · editable engineering knowledge</small></div><div className="ecosystem-action action-discover"><i>◌</i><b>DISCOVER</b><small>New projects<br />& creators</small></div><div className="ecosystem-action action-share"><i>◇</i><b>SHARE</b><small>Publish your<br />structured work</small></div><div className="ecosystem-action action-fork"><i>⌘</i><b>FORK</b><small>New project<br />lineage</small></div><div className="ecosystem-action action-remix"><i>△</i><b>REMIX</b><small>Explore new<br />directions</small></div><div className="ecosystem-action action-collaborate"><i>◉</i><b>COLLABORATE</b><small>Work with<br />specialists</small></div><div className="ecosystem-action action-hire"><i>♧</i><b>HIRE</b><small>Makers, suppliers<br />& factories</small></div><div className="creator-lineage">CREATOR ATTRIBUTION <b>Alex Chen · original project</b><small>Forks preserve credit, history, and editable garment intelligence.</small></div></div></div>}
        ...
        <aside className="action-panel">
          ...
          {key === "community" && <><h3>Community ecosystem</h3><p>Structured garment projects carry creator attribution, editable engineering knowledge, and an open lineage.</p><div className="community-actions"><button>Fork garment</button><button>Request collaboration</button><button>Hire maker / factory</button><button>Save to collection</button></div><div className="community-metrics"><b>12<small>forks</small></b><b>4<small>collaborators</small></b><b>3<small>material matches</small></b></div><button className="action-primary" onClick={() => go(25)}>Continue to ending <ArrowRight size={14} /></button></>}
