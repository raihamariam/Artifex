...
function Journey({ onBack, onJoin }: { onBack: () => void; onJoin: () => void }) {
  ...
  return (
    <main className={`journey-page immersive-journey env-${key}`}>
      ...
      <div className="immersive-progress"><span>{String(active + 1).padStart(2, "0")} / {journeyChapters.length}</span><TimelineProgress active={active} /><span>{chapter.environment} · one valid route</span></div>
