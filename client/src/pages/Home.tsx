...
function ReferenceBoard({ chapterKey }: { chapterKey: JourneyKey }) {
  ...
  if (is("engineering", "pattern", "impact", "xray")) return <div className="reference-board reference-engineering"><div className="ref-engineering-model"><Garment mode="studio" material="silver" pattern={chapterKey !== "engineering"} /><span>SCULPTURAL COLLAR</span><span>VOLUMETRIC SLEEVE</span><span>WAIST TWIST</span><span>ASYMMETRIC DRAPE</span></div><div className="ref-pattern-grid">{Array.from({ length: 12 }).map((_, i) => <i key={i} />)}</div><div className="ref-engineering-tabs"><b>3D GARMENT</b><b>SEAM TOPOLOGY</b><b className="active">20 PATTERN PIECES</b><b>CONSTRUCTION</b></div></div>;
  ...
  if (is("release", "release-ready", "ending")) return <div className="reference-board reference-release"><div className="ref-connected-product"><Garment mode="studio" material="silver" /><b>ARTIFEX HERO 001</b></div><div className="ref-release-links">{["Creative intent", "3D model", "2D pattern", "Fit data", "Material spec", "Construction", "BOM & costing", "Tech pack", "QC & testing", "Version history"].map((item) => <span key={item}>● {item}<em>Ready</em></span>)}</div><div className="ref-release-status"><b>RELEASE PACKAGE</b><span>All connected product intelligence</span><strong>APPROVED</strong></div></div>;
  if (chapterKey === "community") return <div className="reference-board reference-community"><div className="ref-community-feature"><Garment mode="mini" material="silver" /><b>ARTIFEX HERO 001</b><small>by Artifex · structured project</small></div>{["Ready-to-wear fork", "Drape study", "Material study", "Editorial remix"].map((item, i) => <div className="ref-community-card" key={item}><Garment mode="mini" material={i === 2 ? "matte" : "silver"} /><span>{item}</span><small>♡ {1.2 - i * .2}k · Fork</small></div>)}</div>;
